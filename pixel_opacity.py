...
with open("2022_place_canvas_history.sorted.csv", encoding="utf8") as file:
    for line in tqdm(file, total=TOTAL_PLACEMENTS):
        ...
        # Blend new colour with previous colour based on user activity
        canvas[coordinate[1], coordinate[0]] = tuple(
            int((p * (1 - OPACITY)) + (n * OPACITY))
            for n, p in zip(color, canvas[coordinate[1], coordinate[0]])
        )
